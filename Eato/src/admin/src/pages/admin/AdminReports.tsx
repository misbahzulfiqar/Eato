...
type TrendRow = { _id: string; orders?: number; revenue?: number };
type RestaurantPerfRow = { _id?: string; restaurantName?: string; orders?: number; revenue?: number };
type ReportData = {
  summary?: { orders?: number; revenue?: number; delivered?: number; cancelled?: number };
  trend?: TrendRow[];
  restaurants?: RestaurantPerfRow[];
};
...
export default function AdminReports() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const d = (await adminApi.reports(period)) as ReportData;
      setData(d);
    } catch (e: unknown) {
