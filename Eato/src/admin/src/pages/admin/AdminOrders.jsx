...
function pickOrderPreviewUrl(o) {
  for (const it of o.items || []) {
    const mi = it.menuItemId;
    if (mi && typeof mi === 'object' && mi.imageUrl) return mi.imageUrl;
  }
  const rest = o.restaurantId;
  if (rest && typeof rest === 'object' && rest.imageUrl) return rest.imageUrl;
  return '';
}
...
const STATUSES = ['', 'pending', 'processing', 'packing', 'shipping', 'delivered', 'cancelled'];
...
export default function AdminOrders() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setOrders(await adminApi.orders(params));
    } catch (e) {
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <button type="button" onClick={load} className="rounded-lg border border-stone-200 px-3 py-1.5 text-sm font-semibold text-stone-700 hover:bg-stone-50">
          Refresh
        </button>
      </div>
                ...
                <div className="border-t border-stone-100 bg-stone-50 p-4 text-sm">
                  ...
                  <div className="mt-4 rounded-lg border border-stone-200 bg-white p-3">
                    ...
                    <label className="mt-2 flex items-center gap-2 text-sm">
                      <input type="checkbox" checked={notesDraft.disputeOpen} onChange={(e) => setNotesDraft((d) => ({ ...d, disputeOpen: e.target.checked }))} />
                      Flag dispute open
                    </label>
