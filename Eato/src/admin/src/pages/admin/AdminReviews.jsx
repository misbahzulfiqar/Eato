...
function pickReviewImageUrl(rev) {
  const dish = rev.menuItemId;
  if (dish && typeof dish === 'object' && dish.imageUrl) return dish.imageUrl;
  const rest = rev.restaurantId;
  if (rest && typeof rest === 'object' && rest.imageUrl) return rest.imageUrl;
  return '';
}

function ReviewPreviewThumb({ url }) {
...
function ratingCardClasses(rating) {
