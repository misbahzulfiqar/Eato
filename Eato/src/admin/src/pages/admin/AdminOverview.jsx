...
export default function AdminOverview() {
  ...
  const [reportsWeek, setReportsWeek] = useState(null);
  const [chartsData, setChartsData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [notifications, setNotifications] = useState(() => readJson(NOTIF_KEY, []));
  ...
  useEffect(() => {
    ...
    const pollNotifications = async () => {
      try {
        ...
        if (charts) setChartsData(charts);

        const restaurantIds = restaurantList.map((r) => String(r._id));
        ...
        const seenRestaurants = new Set(readJson(SEEN_RESTAURANTS_KEY, []));
        const seenOrders = new Set(readJson(SEEN_ORDERS_KEY, []));
