...
export default function RestaurantRegister() {
  ...
  const [form, setForm] = useState({
    name: '', email: '', password: '', restaurantName: '', description: '', address: '', phone: '', cuisine: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      const { user, token } = await auth.registerRestaurant(form);
  ...
  return (
    <div className="max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" className="w-full py-2 bg-eato-orange text-white rounded-lg font-medium hover:bg-orange-600">Register restaurant</button>
      </form>
      <p className="mt-4 text-stone-600">Already have an account? <Link to="/login/restaurant" className="text-eato-orange font-medium">Login</Link></p>
    </div>
  );
}
