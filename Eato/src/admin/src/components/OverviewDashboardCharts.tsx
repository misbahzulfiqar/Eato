...
function formatDayLabel(isoDate: string | undefined) {
  if (!isoDate) return '';
...
function OrderPreviewThumb({ url }: { url: string | undefined }) {
...
type DailyRow = { date?: string; orders?: number; delivered?: number; cancelled?: number; label?: string };
type StatusRow = { status: string; count: number };
type RestaurantShareRow = { name: string; orderCount: number };
type RecentOrderRow = {
  id: string;
...
export type OverviewChartData = {
  ...
  orderStatusBreakdown?: StatusRow[];
  restaurantShare?: RestaurantShareRow[];
  recentOrders?: RecentOrderRow[];
};

type TopItemRow = { name?: string; quantitySold?: number };
...
export default function OverviewDashboardCharts({ chartData, topItems }: Props) {
  ...
  const lineData: DailyRow[] = dailyActivity.map((d) => ({
    ...d,
  ...
  const renderTooltip = ({ active, payload }: TooltipProps<number, string>) => {
    ...
    return (
      <div className="rounded-lg border border-stone-200 bg-white px-3 py-2 text-xs shadow-lg">
        {payload.map((p) => (
          <div key={String(p.dataKey)} className="font-medium text-stone-800">
                ...
                <PieChart>
                  <Tooltip
                    formatter={(value, _name, props) => [
                      ...
                      (props as { payload?: { fullName?: string; name?: string } }).payload?.fullName ||
                        (props as { payload?: { fullName?: string; name?: string } }).payload?.name,
                    ]}
                  />
